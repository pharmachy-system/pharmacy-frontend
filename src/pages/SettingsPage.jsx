            ...
            {[['ar', 'العربية', 'RTL'], ['en', 'English', 'LTR']].map(([l, label, dir]) => (
              <button key={l} onClick={() => setLang(l)}
                className={`flex-1 py-3 rounded-xl border-2 font-semibold text-sm transition-colors ${
                  lang === l ? 'border-cyan-500 bg-cyan-50 text-cyan-700' : 'border-gray-200 text-gray-600 hover:border-gray-300'
                }`}>
                {label}
            ...
            {[
              ...
              <label key={key} className="flex items-center justify-between cursor-pointer py-1">
                ...
                <div onClick={() => setNotifs(n => ({ ...n, [key]: !n[key] }))}
                  className={`relative w-11 h-6 rounded-full transition-colors ${notifs[key] ? 'bg-cyan-500' : 'bg-gray-200'}`}>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
          ...
          <div className="space-y-3">
            ...
            <button onClick={changePassword} disabled={pwSaving}
              className="flex items-center gap-2 px-4 py-2.5 bg-cyan-600 hover:bg-cyan-700 text-white rounded-xl text-sm font-semibold disabled:opacity-50">
